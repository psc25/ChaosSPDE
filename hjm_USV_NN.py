...
NN_der_0 = tf.reduce_sum(Y*act_der(hid_0)*A1, axis = 1)
X_IJK_der_0_tf = tf.reduce_sum(tf.expand_dims(NN_der_0, 1)*tf.reshape(xi_alpha_tf, [nr_alpha, -1, 1, 1]), axis = 0)

NN_der2 = tf.reduce_sum(Y*act_der2(hid)*tf.square(A1), axis = 1)
X_IJK_der2_tf = tf.reduce_sum(tf.expand_dims(NN_der2, 1)*tf.reshape(xi_alpha_tf, [nr_alpha, -1, 1, 1]), axis = 0)

# True solution
X_tf = tf.expand_dims(r_tf, -1)*tf.exp(-kappa*uu1_tf[0]) + mu/kappa*(1.0-tf.exp(-kappa*uu1_tf[0])) - sigma**2/(2.0*kappa**2)*tf.square(1.0-tf.exp(-kappa*uu1_tf[0]))
...
# Loss function
initl_0 = X_0_tf[:, 0:1]
drift_0 = tf.cumsum(X_IJK_der_0_tf, axis = 1)*dt
diffu_0 = sigma*tf.expand_dims(W_ij_tf[0], -1)
loss_train1 = tf.reduce_mean(tf.square(X_IJK_0_tf - (initl_0 + drift_0 + diffu_0)))

initl_der = X_der_tf[:, 0:1]
drift_der = tf.cumsum(X_IJK_der2_tf + sigma**2*(2.0*tf.exp(-2.0*kappa*uu1_tf[0]) - tf.exp(-kappa*uu1_tf[0])), axis = 1)*dt
diffu_der = -sigma*kappa*tf.exp(-kappa*uu1_tf[0])*tf.expand_dims(W_ij_tf[0], -1)
loss_train2 = tf.reduce_mean(tf.square(X_IJK_der_tf - (initl_der + drift_der + diffu_der)))
...
for i in range(ep):
    ...
    for j in range(nrbatch):
        ...
        feed_dict = {xi_alpha_tf: xi_alpha[:, ind_batch], tt1_tf: tt1, uu1_tf: uu1, W_ij_tf: W_ij[:, ind_batch], r_tf: r[ind_batch]}
    ...
    if (i+1) % eval_every == 0:
        begin = time.time()
        feed_dict = {xi_alpha_tf: xi_alpha[:, ind_test], tt1_tf: tt1, uu1_tf: uu1, W_ij_tf: W_ij[:, ind_test], r_tf: r[ind_test]}
        ...
        feed_dict = {xi_alpha_tf: xi_alpha[:, ind_plot:(ind_plot+1)], tt1_tf: tt1, uu1_tf: uuplot, W_ij_tf: W_ij[:, ind_plot:(ind_plot+1)], r_tf: r[ind_plot:(ind_plot+1)]}
