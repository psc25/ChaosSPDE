...
for i in range(ep):
    ...
    for j in range(nrbatch):
        ...
        feed_dict = {xi_alpha_tf: xi_alpha[:, ind_batch], tt1_tf: tt1, uu1_tf: uu1, X_tf: X[ind_batch]}
    ...
    if (i+1) % eval_every == 0:
        begin = time.time()
        feed_dict = {xi_alpha_tf: xi_alpha[:, ind_test], tt1_tf: tt1, uu1_tf: uu1, X_tf: X[ind_test]}
        ...
        feed_dict = {xi_alpha_tf: xi_alpha[:, ind_plot:(ind_plot+1)], tt1_tf: tt1, uu1_tf: uuplot, X_tf: X_plot}
...
np.savetxt(path + "/" + name + "_" + str(J) + "_X_true.csv", np.reshape(X_true_plot, [M2, -1]))
